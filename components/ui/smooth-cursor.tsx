...
import { FC, JSX, useEffect, useRef, useState, isValidElement, cloneElement } from "react";
...
const DefaultCursorSVG: FC<{ dark?: boolean }> = ({ dark = false }) => {
  // dark = true  -> render white fill (for dark theme)
  // dark = false -> render black fill (for light theme)
  const fillColor = dark ? "#FFFFFF" : "#000000";
  const strokeColor = dark ? "#000000" : "#FFFFFF";
...
export function SmoothCursor({
  ...
  useEffect(() => {
    // saat component ter-mount tambahkan class agar CSS global bisa menyembunyikan cursor
    const prevCursor = document.body.style.cursor;
    document.body.classList.add('smooth-cursor-active');
    // keep original inline cursor to restore on unmount
    return () => {
  ...
  // remove internal project-card mouseenter/leave listeners (page.tsx handles mount/unmount),
  // keep mouse movement handling below...
  useEffect(() => {
